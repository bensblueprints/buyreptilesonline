...
router.post('/products', requireAdmin, upload.single('image'), (req, res) => {
  const { name, scientific_name, description, price, compare_price, cost_price, supplier, category_id, stock, featured, active, sex, age, morph, weight } = req.body;
  const slug = name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
  const image = req.file ? '/uploads/' + req.file.filename : null;
  const result = req.db.prepare(`
    INSERT INTO products (name, scientific_name, slug, description, price, compare_price, cost_price, supplier, category_id, image, stock, featured, active, sex, age, morph, weight)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(name, scientific_name || null, slug, description, parseFloat(price), compare_price ? parseFloat(compare_price) : null,
...
router.put('/products/:id', requireAdmin, upload.single('image'), (req, res) => {
  ...
  req.db.prepare(`
    UPDATE products SET name=?, scientific_name=?, slug=?, description=?, price=?, compare_price=?, cost_price=?, supplier=?, category_id=?, image=?, stock=?, featured=?, active=?, sex=?, age=?, morph=?, weight=?
    WHERE id=?
  `).run(name, scientific_name || null, slug, description, parseFloat(price), compare_price ? parseFloat(compare_price) : null,
    cost_price ? parseFloat(cost_price) : null, supplier || null,
    parseInt(category_id) || null, image, parseInt(stock) || 0, featured === 'true' || featured === '1' ? 1 : 0,
